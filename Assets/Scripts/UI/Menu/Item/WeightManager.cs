...
public class WeightManager
{
    ...
    public static int Max
    {
        ...
        {
            int max = 0;
            var party = SaveManager.instance.CurrentSave.currentParty;
    ...
    public static float Sum
    {
        get
        {
            ...
            float sum = 0;
            var items = SaveManager.instance.CurrentSave.itemInCharacter;
            foreach (var i in items)
            {
                var item = Database.items.Find(x => x.itemName == i.itemName);
                if (item == null)
                {
                    Debug.LogWarning($"[WeightManager] 아이템을 찾을 수 없음: {i.itemName}");
                    continue;
                }
                sum += item.weight;
            }
